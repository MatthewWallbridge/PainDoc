...
function NoteCard({ note, onDelete }) {
  ...
  const s = {
    ...
    deleteBtn: {
      ...
      color: 'var(--text-hint)', cursor: 'pointer', padding: 4,
    },
    chevron: { color: 'var(--text-hint)', flexShrink: 0 },
    body: {
    ...
    sectionLabel: {
      ...
      color: '#1D9E75', fontWeight: 'bold', marginBottom: '0.3rem', marginTop: '0.75rem',
    },
    sectionContent: {
    ...
    <div style={s.card}>
      <div style={s.header} onClick={() => setExpanded(v => !v)}>
        ...
        <div style={s.actions}>
          ...
          <button style={s.deleteBtn} onClick={e => { e.stopPropagation(); onDelete(note.id) }}>
            <Trash2 size={14} />
          </button>
          <span style={s.chevron}>{expanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</span>
        </div>
      </div>
      {expanded && (
        <div style={s.body}>
          {SECTIONS.map((sec, i) => (
            <div key={sec.key}>
              {i > 0 && <div style={s.sectionLabel}>{sec.label}</div>}
              {i === 0 && <div style={{ ...s.sectionLabel, marginTop: 0 }}>{sec.label}</div>}
...
export default function NoteHistory({ notes, onDelete, onClearAll }) {
  ...
  const s = {
    ...
    label: {
      ...
      color: 'var(--text-hint)', display: 'flex', alignItems: 'center', gap: 6,
    },
    dot: { width: 6, height: 6, borderRadius: '50%', background: 'var(--accent)', display: 'inline-block' },
    clearBtn: {
      fontSize: '0.68rem', fontFamily: 'Courier New, monospace',
      color: 'var(--text-hint)', background: 'none',
